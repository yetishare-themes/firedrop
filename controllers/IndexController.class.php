...
namespace Themes\Firedrop\Controllers;
...
class IndexController extends CoreIndexController
{
    ...
    // load template
    return $this->render('network_status.html', array(
        'indexVariable' => 'Network Status',
    ));
}
...
// page signup
	...
	public function signup($packageId = null) {
        ...
        if ($packageId === null) {
            ...
            if (count($premiumPackages) === 1) {
                foreach ($premiumPackages AS $premiumPackage) {
                    return $this->redirect(WEB_ROOT . '/signup/' . $premiumPackage['id']);
                }
            }
            ...
            foreach ($premiumPackages AS $k => $premiumPackage) {
                ...
                switch ($premiumPackage['level_type']) {
                    ...
                    case 'paid':
                        ...
                        $pricingButton = array(
                            'url' => CoreHelper::getCoreSitePath() . '/signup/' . $premiumPackage['id'],
                            'label' => $Auth->loggedIn() ? TranslateHelper::t('upgrade_now', 'Upgrade Now') : TranslateHelper::t('signup_now', 'Signup Now'),
                        );
